...
struct Summarizer {
public:
  Summarizer(bool full_log = false)
      : full_log(full_log), num_data(0), sum(0.0), sq_sum(0.0), last_x(0.0) {}
  ...
  double median() const {
    ...
    std::nth_element(sorted.begin(), sorted.end(),
...
public:
  explicit KittiDataset(const std::string &dataset_path,
    ...
    std::transform(
        filenames.begin(), filenames.end(), points.begin(),
        [](const std::string &filename) { return read_points(filename); });
  }
  ...
  std::vector<std::shared_ptr<PointCloud>> convert(bool release = false) {
    ...
    std::transform(points.begin(), points.end(), converted.begin(),
                   [=](auto &raw_points) {
                     auto points = std::make_shared<PointCloud>();
                     traits::resize(*points, raw_points.size());
                     for (size_t i = 0; i < raw_points.size(); i++) {
                       traits::set_point(*points, i,
                                         raw_points[i].template cast<double>());
                     }
